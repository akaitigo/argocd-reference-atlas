...
| `failure-recovery` | Failure／Recovery | diagnosis, recovery | `failure.degraded-dependency`, `recovery.control-plane-restore`, `recovery.automated-resynchronization` | `health.resource-assessment`, `reconciliation.continuous-loop` | `failure-recovery`, `operations-observability` | 障害／Restoreはcovered。自動再同期Targetはpartial |
