...
description: Argo CD v3.5.2のArchitecture、ApplicationSet、接続、同期、Security、HA、Notifications、Observability、障害復旧、移行、運用に関する設計・実装・診断・レビューを、AtlasのCoverage、Claim、Runbook、ADR、Failure mode、Evidenceへ案内する。Coverage外の機能説明には使わない。
---
...
- `coverage.yaml`のTargetが`covered`である場合だけ、そこに列挙された`claim_ids`と`evidence_ids`をTarget全体の実証済み根拠として使います。`partial`のpass Evidenceは、記録された固定FixtureとClaimに限定したbounded observationとして引用できますが、Target closureや一般保証には使いません。`planned`、`missing`、`expired`は不足を明示します。
...
- `evidence/certificates/historical/`のCertificateは記録されたsource commitだけを対象とします。current HEADのactive completionやDefinitive完成へ流用しません。
...
3. Definitiveの細粒度Surfaceは`definitive/surface-inventory.yaml`で確認します。Targetが`partial`または`missing`なら`definitive/gap-ledger.yaml`のScenarioを返し、広いTarget名だけでclosureを判断しません。Authority completenessを問われた場合、既存Feature／Source edgeは`authority/extraction.snapshot.json`、unique documentから列挙したraw anchor候補は`authority/body-inventory.snapshot.json`、人手確認待ちは`authority/review-queue.snapshot.json`、昇格Decisionは`authority/reviews/decisions.json`で別々に確認します。priority／candidate cluster／batchは提案にすぎません。人による一次資料確認とreviewer／time／reason／digest／locator／Mapping／resultの整合がないanchor、またはstale hold中のdocumentをController／Behavior itemへ昇格せず、Queue件数をSemantic Surface、Coverage、Depth達成へ算入しません。非後退は`baselines/authority-body-inventory-v1.json`で確認します。
4. 完成度を問われた場合は`definitive/argocd-depth-parity.json`のFE Depth Reference由来18軸と`definitive/fe-parity-matrix.json`のArgo CD機能20軸を確認します。どちらかにopen denominatorまたは`gap_ids`が一つでもあればDefinitiveと表現しません。公開mainからの非後退は`baselines/public-main-v0.1.0.non-regression.json`と`evidence/migrations/`で確認します。
5. そのTargetに接続されたClaimだけを`atlas/claims/`から読み、対応するSource IDを`sources.lock.yaml`で確認します。
