...
def main() -> None:
    ...
    rejected("runtime-completed-forged", lambda value: value["denominator"].update(runtime_execution_completed_rows=14))
    rejected("completion-remaining-ambiguous", lambda value: value["denominator"].update(completion_remaining_rows=987))
    ...
    rejected("runtime-credit-promoted", lambda value: value["closure"].update(runtime_only_rows_without_completion_credit=0))
    ...
    print("Root Verification Matrix fixtures passed: positive=1 negative=17")
